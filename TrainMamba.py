"""
Subject-dependent training for the Mamba-backboned DLSSNet variant
(Model/DLSSNet/model_mamba.py) -- same corrected protocol, same data split,
same losses as TrainCorrectedProtocol.py, so results are directly comparable
to that 66.94% baseline. Only the backbone differs (real selective SSM
instead of plain convs + single-head attention).

Usage:
    DLSSNET_DATA_PATH=/path/to/BCICIV_2a_mat DLSSNET_GPU=0 \
        python TrainMamba.py [subject_ids...]
"""
import json
import os
import random
...
from Model.DLSSNet.args_mamba import data_config
...
def build_model():
    return Net(
        ...
        ssm_state_dim=data_config.ssm_state_dim,
        mamba_expand=data_config.mamba_expand,
        mamba_conv_kernel=data_config.mamba_conv_kernel,
        freq_kernel_sizes=data_config.freq_kernel_sizes,
        tre_hidden=data_config.tre_hidden,
    )
