...
def save_checkpoint(net=None, optimizer=None, epoch=None, train_losses=None, train_acc=None, val_loss=None,
                    val_acc=None, check_acc=None, savepath=None,m_name=None, GPUdevices=1):
    if GPUdevices > 1:
    ...
    if val_acc >= check_acc:
        savepath = savepath + '/{}_best_params.pkl'.format(m_name)
        check_acc = val_acc
        print('Current Best (top-1 accuracy):', check_acc)
    else:
...
def adjust_learning_rate(optimizer, epoch, model_type, data_config):
