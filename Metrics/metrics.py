...
"""
"""
只能保证 accuracy 和balanced accuracy 可用。目前剩下的还没有修改
...
def balanced_accuracy(output, target):
    ...
    for c in range(num_classes):
        true_positives[c] = torch.sum((y_true == c) & (y_pred == c))
        a = (y_true == c) & (y_pred == c)
        false_positives[c] = torch.sum((y_true != c) & (y_pred == c))
        true_negatives[c] = torch.sum((y_true != c) & (y_pred != c))
        false_negatives[c] = torch.sum((y_true == c) & (y_pred != c))

    # 计算每个类别的准确率
    sensitivities = true_positives / (true_positives + false_negatives + 1e-12)
    specificities = true_negatives / (true_negatives + false_positives + 1e-12)

    # 计算平衡准确率
    balanced_acc = (sensitivities + specificities) / 2

    # 返回平衡准确率的平均值
    return torch.mean(balanced_acc)*100

def accuracy(output:torch.Tensor, target):
    ...
    _, pred = output.topk(1, 1, True, True)  # [128, 5],indices
...
def accuracytop(output, target, topk=(1,2)):
    """Computes the precision@k for the specified values of k
        准确率
    """  # [128, 10],128
    maxk = max(topk)
...
if __name__ == "__main__":
    ...
    LRAP, map, samp_acc, macro_acc, micro_acc, rec, f1 = acc(aa, bb)
